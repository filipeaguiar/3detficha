...
#### Scenario: Render techniques tab with the same visual system
- **WHEN** a user opens the techniques tab
- **THEN** technique acquisition, owned technique cards, strike selection, metadata rows, and action areas SHALL follow the same editor composition system used by the other editor tabs instead of mixing legacy row styling with the newer card system.
